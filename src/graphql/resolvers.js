...
export const resolvers = {
	Query: {
		...
		getCompany: async (_, { companyId }) => CompaniesModel.findOne({ 
			companyId 
		}),
		...
		getEmployees: async (_, { companyId }) => {
			// .populate('employees', 'employeeId name position email');
	...
	Mutation: {
		...
		createCompany: async (_, { name, employee }) => {
			...
			// Verifica se existem dados do employee
			if (!employee) {
				throw new Error('Funcionário inicial é obrigatório');
			}
			...
			// Cria o primeiro usuário da empresa
			...
			await newCompany.save(); // Salva a empresa

			newEmployee.companyId = newCompany._id;
			await newEmployee.save(); // Salva o funcionário

			return await CompaniesModel.findById(newCompany._id).populate('employees', 'employeeId name position email'); // Com populate define quais dados do employee vai retornar
		},
		...
		login: async (_, { email, password }, { res }) => {
			// Verifica se o email já existe no banco de dados
			const employee = await EmployeesModel.findOne({ email }).populate('company');
			const employeePassCheck = await EmployeesModel.findOne({ email }).select("+password");
			
			// Verifica senha
			const passwordMatch = await bcrypt.compare(password, employeePassCheck.password);
			if (!passwordMatch) {
